...
sku_create_node(uint32_t fuserev_low,
		...
		uint32_t mem_size, char *sku_name,
		sku_info_t ** newnode)
{
	sku_info_t *temp;
...
sku_find(mic_ctx_t *mic_ctx, uint32_t device_id)
{
	...
	struct list_head skulist_in;
	struct list_head skulist_out;
	uint32_t fuse_rev, memsize;
	struct list_head *pos, *tmp;
	...
	list_for_each_safe(pos, tmp, &mic_data.sku_table[device_id]) {
		...
		if ((match->fuserev_low <= fuse_rev) && (match->fuserev_high >= fuse_rev)) {
			...
			ret = sku_create_node(match->fuserev_low, match->fuserev_high,
					match->memsize, match->sku_name, &newnode);
			if (ret) {
				strncpy(mic_ctx->sku_name, invalid, SKU_NAME_LEN - 1);
				mic_ctx->sku_name[SKU_NAME_LEN - 1] = '\0';
				goto cleanup;
	...
	sku_swap_list(&skulist_out, &skulist_in);
	/* Search by memsize */
	...
	cnt = 0;
	list_for_each_safe(pos, tmp, &skulist_in) {
		match = list_entry(pos, sku_info_t, sku);
...
sku_build_table(mic_ctx_t *mic_ctx)
{
	...
	/*2250*/
	sku_create_node(0, 1, SKU_LOW_MEM, "A0PO-SKU1", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(0, 1, SKU_HIGH_MEM, "A0PO-SKU1", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(2, 3, SKU_LOW_MEM, "ES1-SKU2", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(2, 3, SKU_HIGH_MEM, "ES1-SKU2", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(4, 49, SKU_HIGH_MEM, "ES1B-SKU2", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(50, 100, SKU_HIGH_MEM, "B0PO-SKU2", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(101, 150, SKU_HIGH_MEM, "ES2-P1640", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(153, 154, SKU_HIGH_MEM, "B1PO-5110P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(151, 152, SKU_HIGH_MEM, "B1PO-P1640", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(156, 156, SKU_HIGH_MEM, "B1PRQ-5110P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(155, 155, SKU_HIGH_MEM, "B1QS-5110P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(157, 157, SKU_HIGH_MEM, "B1PRQ-5110P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(158, 159, SKU_HIGH_MEM, "B1PRQ-5110P/5120D", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(201, 202, SKU_HIGH_MEM, "B2QS-5110P/5120D", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(201, 203, SKU_HIGH_MEM, "B2PRQ-5110P/5120D", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(251, 254, SKU_HIGH_MEM, "C0-5110P/5120D", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);
	sku_create_node(255, 300, SKU_HIGH_MEM, "C0 QS-5110P/5120D", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[0]);

	/*2251*/
	sku_create_node(0, 1, SKU_LOW_MEM, "A0PO-SKU2", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[1]);
	sku_create_node(0, 1, SKU_HIGH_MEM, "A0PO-SKU2", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[1]);

	/*2252*/
	sku_create_node(0, 1, SKU_LOW_MEM, "A0PO-SKU3", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[2]);

	/*2253*/
	sku_create_node(0, 1, SKU_LOW_MEM, "A0PO-SKU4", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[3]);
	sku_create_node(2, 3, SKU_LOW_MEM, "ES1-SKU5", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[3]);
	sku_create_node(4, 49, SKU_LOW_MEM, "ES1B-SKU5", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[3]);
	sku_create_node(50, 100, SKU_LOW_MEM, "B0PO-SKU5", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[3]);

	/*2254*/

	/*2255*/
	sku_create_node(0, 1, SKU_LOW_MEM, "A0PO-SKUX", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[5]);

	/*2256*/
	sku_create_node(0, 1, SKU_LOW_MEM, "A0PO-SKU5", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[6]);

	/*2257*/
	sku_create_node(0, 1, SKU_LOW_MEM, "A0PO-SKUZ", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[7]);

	/*2258*/
	sku_create_node(2, 3, SKU_LOW_MEM, "ES1-SKU1", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[8]);
	sku_create_node(2, 3, SKU_HIGH_MEM, "ES1-SKU1", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[8]);
	sku_create_node(4, 49, SKU_HIGH_MEM, "ES1B-SKU1", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[8]);
	sku_create_node(50, 100, SKU_HIGH_MEM, "B0PO-SKU1", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[8]);

	/*2259*/
	sku_create_node(2, 3, SKU_LOW_MEM, "ES1-SKU3", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[9]);
	sku_create_node(2, 3, SKU_HIGH_MEM, "ES1-SKU3", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[9]);

	/*225A*/
	sku_create_node(2, 3, SKU_LOW_MEM, "ES1-SKU4", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[10]);
	sku_create_node(4, 49, SKU_LOW_MEM, "ES1B-SKU4", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[10]);
	sku_create_node(50, 100, SKU_LOW_MEM, "B0PO-SKU4", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[10]);
	sku_create_node(101, 150, SKU_LOW_MEM, "ES2-SKU4", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[10]);

	/*225B*/
	sku_create_node(4, 49, SKU_HIGH_MEM, "ES1B-SKU3cs", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[11]);
	sku_create_node(4, 49, SKU_LOW_MEM, "ES1B-SKU3ncs", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[11]);
	sku_create_node(50, 100, SKU_HIGH_MEM, "B0PO-SKU3cs", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[11]);
	sku_create_node(50, 100, SKU_LOW_MEM, "B0PO-SKU3ncs", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[11]);

	/*225C*/
	sku_create_node(101, 150, SKU_HIGH_MEM, "ES2-P/A/X 1750", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[12]);
	sku_create_node(153, 154, SKU_HIGH_MEM, "B1PO-7110 P/A", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[12]);
	sku_create_node(155, 155, SKU_HIGH_MEM, "B1QS-7110 P/A", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[12]);
	sku_create_node(151, 152, SKU_HIGH_MEM, "B1PO-P/A 1750", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[12]);
	sku_create_node(156, 156, SKU_HIGH_MEM, "B1PRQ-7110 P/A/X", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[12]);
	sku_create_node(157, 157, SKU_HIGH_MEM, "B1PRQ-7110 P/A/X", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[12]);
	sku_create_node(158, 159, SKU_HIGH_MEM, "B1PRQ-7110 P/X", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[12]);
	sku_create_node(201, 202, SKU_HIGH_MEM, "B2QS-7110 P/X", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[12]);
	sku_create_node(201, 203, SKU_HIGH_MEM, "B2PRQ-SE10 P/X", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[12]);
	sku_create_node(251, 350, SKU_HIGH_MEM, "C0-7120 P/A/X/D", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[12]);

	/*225D*/
	sku_create_node(101, 150, SKU_LOW_MEM, "ES2-P1310", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);
	sku_create_node(101, 150, SKU_HIGH_MEM, "ES2-A1330", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);
	sku_create_node(153, 154, SKU_LOW_MEM, "B1PO-3110P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);
	sku_create_node(153, 154, SKU_HIGH_MEM, "B1PO-3115A", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);
	sku_create_node(157, 157, SKU_LOW_MEM, "B1PRQ-3110P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);
	sku_create_node(157, 157, SKU_HIGH_MEM, "B1PRQ-3115A", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);
	sku_create_node(156, 156, SKU_LOW_MEM, "B1PRQ-3110P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);
	sku_create_node(156, 156, SKU_HIGH_MEM, "B1PRQ-3115A", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);
	sku_create_node(155, 155, SKU_HIGH_MEM, "B1QS-3115A", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);
	sku_create_node(155, 155, SKU_LOW_MEM, "B1QS-3110P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);
	sku_create_node(158, 159, SKU_HIGH_MEM, "B1PRQ-3120P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);
	sku_create_node(201, 202, SKU_HIGH_MEM, "B2QS/3120P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);
	sku_create_node(201, 203, SKU_HIGH_MEM, "B2PRQ-3120P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);
	sku_create_node(251, 254, SKU_HIGH_MEM, "C0-3120 P/A", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);
	sku_create_node(255, 300, SKU_HIGH_MEM, "C0 QS-3120 P/A", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[13]);

	/*225E*/
	sku_create_node(157, 157, SKU_HIGH_MEM, "B1PRQ-31S1P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[14]);
	sku_create_node(158, 159, SKU_HIGH_MEM, "B1PRQ-31S1P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[14]);
	sku_create_node(201, 202, SKU_HIGH_MEM, "B2QS-31S1P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[14]);
	sku_create_node(201, 203, SKU_HIGH_MEM, "B2PRQ-31S1P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[14]);
	sku_create_node(251, 254, SKU_HIGH_MEM, "C0-31S1P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[14]);
	sku_create_node(255, 300, SKU_HIGH_MEM, "C0 QS-31S1P", &newnode);
	list_add_tail(&newnode->sku, &mic_data.sku_table[14]);

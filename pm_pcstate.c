 ...
 * @param - size - the size(in bytes) of the nodemask buffer
 *
 * prints the nodes in the nodemask.
 *
 * @returns - none
 */
void print_nodemaskbuf(uint8_t* buf, uint64_t size) {
...
int do_idlestate_entry(mic_ctx_t *mic_ctx)
{
	...
	print_nodemaskbuf(nodemask_buf, mic_ctx->micpm_ctx.nodemask.len);

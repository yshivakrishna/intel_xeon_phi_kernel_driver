...
load_initramfs(mic_ctx_t *mic_ctx, char *initramfsname, uint32_t *initramfs_image, uint32_t *initramfs_size)
{
	...
	*initramfs_size = (file_size >> 20) + 1; /* Size in whole MB */
	*initramfs_image = file_load_offset;
	...
	// transfer uOs image file to gddr
	status = mic_load_file(initramfsname, ((uint8_t*)aperture_va) + file_load_offset, file_size);
	return status;
}
#endif
...
int
load_command_line(mic_ctx_t *mic_ctx, uint64_t uos_cmd_offset, uint32_t initramfs_image, uint32_t initramfs_size)
{
	void *cmd_line_va = mic_ctx->aper.va + uos_cmd_offset;
	uint32_t cmdlen = 0;
	char *buf = NULL;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,34)) || defined(__GNUC_RH_RELEASE__)
	...
	if (initramfs_image) {
		cmdlen += snprintf(buf + cmdlen, MIC_CMDLINE_BUFSIZE - cmdlen,
			 " memmap=%dM$0x%08x rdinit=/init initrd=0x%08x,%dM ",
			 initramfs_size, initramfs_image, initramfs_image, initramfs_size);
	}
...
notify_uosboot(mic_ctx_t *mic_ctx, uint32_t uos_size)
{
	...
	status = get_uos_reserved_size(mmio_va, adapter_memsize, &uos_reserved_size);
...
boot_linux_uos(mic_ctx_t *mic_ctx, char *imgname, char *initramfsname)
{
...
#ifdef INITRAMFS
	/* For now, we only support initramfs for KNC */
	if (mic_ctx->bi_family == FAMILY_KNC) {
		if (initramfsname) {
		  if ((status = load_initramfs(mic_ctx, initramfsname, &initramfs_image, &initramfs_size)) != 0) {
	...
	status = load_command_line(mic_ctx, uos_cmd_offset, initramfs_image, initramfs_size);

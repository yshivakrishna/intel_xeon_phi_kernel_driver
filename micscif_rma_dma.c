...
void *ioremap_remote(off_t off, struct reg_range_t *window,
				size_t len, bool loopback, struct micscif_dev *dev)
{
	void *ret;
	dma_addr_t phys =	micscif_get_dma_addr(window, off, NULL, NULL, NULL);
...
#endif
#ifdef CONFIG_MK1OM
			phys = micscif_get_dma_addr(window, off, NULL, NULL, NULL);
#endif
	...
	while (remaining_len) {
		...
		if (RMA_WINDOW_SELF == src_window->type)
			...
			src_virt = ioremap_remote(src_offset,
					src_window, loop_len, work->loopback, work->remote_dev);
		if (!src_virt) {
			ret = -ENOMEM;
			goto error;
		}
		...
		if (RMA_WINDOW_SELF == dst_window->type)
			...
			dst_virt = ioremap_remote(dst_offset,
					dst_window, loop_len, work->loopback, work->remote_dev);
		if (!dst_virt) {

...
static void memcpy_torb(struct micscif_rb *rb, void *header,
...
static void memcpy_fromrb(struct micscif_rb *rb, void *header,
			void *msg, uint32_t size)
{
	/* Need to call two copies if it wraps around */
	uint32_t size1, size2;
...
int micscif_rb_space(struct micscif_rb *rb)
{
	...
	/*
	 * Update the HW read pointer to see if there is more space.
	 * XPU_RACE_CONDITION: Reading a fresh value across the bus.
	 */
	smp_mb(); /* FIXME: only needs LFENCE */
	rb->current_read_offset = *rb->read_ptr;

...
static ssize_t
show_host_revision(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s\n", HOST_REV);
}
static DEVICE_ATTR(host_revision, S_IRUGO, show_host_revision, NULL);

static ssize_t
show_uos_revision(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s\n", UOS_REV);
}
static DEVICE_ATTR(uos_revision, S_IRUGO, show_uos_revision, NULL);
...
static struct attribute *host_attributes[] = {
	...
	&dev_attr_host_revision.attr,
	&dev_attr_uos_revision.attr,
...
show_family(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	static const char KNC[] = "Knights Corner";
	bd_info_t *bdi = dev_to_bdi(dev);
...
set_micstate(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	const char *image;
	int err = 0;
	const char *default_mm_image = "/lib/firmware/mic/RASMM.elf";
	...
	if (!strncmp(buf, "boot", 4)) {
		...
		if (!strncmp(mode, "linux", 5)) {
			mic_ctx->mode = MODE_LINUX;
			image = &mode[6];
		} else if (!strncmp(mode, "elf", 3)) {
			mic_ctx->mode = MODE_ELF;
			image = &mode[4];
			if(mode[3] != ':') {
				mutex_unlock(&mic_ctx->state_lock);
				return -EIO;
		...
		if ((mic_ctx->image = kmalloc(strlen(image) + 1, GFP_KERNEL)) == NULL) {
			mutex_unlock(&mic_ctx->state_lock);
			return -ENOMEM;
		}

		strcpy(mic_ctx->image, image);
		if (mic_ctx->image[strlen(image) - 1] == '\n')
			mic_ctx->image[strlen(image) - 1] = '\0';
		...
		case MODE_LINUX:
			...
			if ((err = boot_linux_uos(mic_ctx, mic_ctx->image, mic_ctx->initramfs)) != 0) {
				printk("boot_linux_uos failed %d\n", err);
				return err;
			}
		...
		case MODE_ELF:
			printk("Booting ELF\n");
			if(strlen(mic_ctx->image) == 0) {
				printk("Error ! no image specified for EFL \n");
				return -EIO;
			}
			if((err = boot_micdev_app(mic_ctx, mic_ctx->image)) != 0) {
				printk("boot_micdev_app failed %d\n", err);
				return err;
			}
			break;
		...
		case MODE_FLASH:
			...
			if(strlen(mic_ctx->image) == 0) {
				strncpy(mic_ctx->image, default_mm_image,
					strlen(default_mm_image) + 1);
				printk("Using default image for mm handler %s\n", 
					    mic_ctx->image);
			}
			if((err = boot_micdev_app(mic_ctx, mic_ctx->image)) != 0) {
				printk("boot_micdev_app failed %d\n", err);
...
static ssize_t
set_initramfs(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	bd_info_t *bdi = dev_to_bdi(dev);
	mic_ctx_t *mic_ctx = &bdi->bi_ctx;
	int err = 0;

	if (mic_ctx->initramfs != NULL)
		kfree(mic_ctx->initramfs);
	if ((mic_ctx->initramfs = kmalloc(strlen(buf) + 1, GFP_KERNEL)) == NULL)
		return -ENOMEM;
	strcpy(mic_ctx->initramfs, buf);

	if (mic_ctx->initramfs[strlen(buf) - 1] == '\n')
		mic_ctx->initramfs[strlen(buf) - 1] = '\0';

	return err;

}
static DEVICE_ATTR(initramfs, S_IRUGO|S_IWUSR, show_initramfs, set_initramfs);
...
show_postcode(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (postcode == 0x0 || postcode == 0xffffffff) {
		printk("Invalid Postcode : %c%c\n", postcode & 0xff, (postcode >> 8) & 0xff);
		ret = micpm_put_reference(mic_ctx);
		if (ret)
			return -EAGAIN;
		return -ENXIO;
	}

...
scif_user_send(scif_epd_t epd, void *msg, int len, int flags)
{
	struct endpt *ep = (struct endpt *)epd;
	/*
	 * sent_len - Total number of bytes sent.
	 * loop_len - Number of bytes to be sent in this iteration.
	 * chunk_len - Number of bytes allocated for temporary
	 *		kernel buffer aka "chunk" which will be reused
	 *		for every iteration.
	 */
	int err = 0, sent_len = 0, loop_len, chunk_len;
	char *tmp = msg;
	int maxlocalbuflen = (1 << (MAX_ORDER + PAGE_SHIFT - 1));

	pr_debug("SCIFAPI send (U): ep %p %s\n", ep, scif_ep_states[ep->state]);

	chunk_len = min(len, maxlocalbuflen);
	...
	if (!(msg = kmalloc(chunk_len, GFP_KERNEL))) {
		err = -ENOMEM;
		goto send_err;
	}
	...
	while (sent_len != len) {
		loop_len = min(maxlocalbuflen, len - sent_len);

		if (copy_from_user(msg, tmp, loop_len)) {
			err = -EFAULT;
			goto send_err_free;
		}
		if ((err = _scif_send(epd, msg,
				loop_len, flags)) < 0)
			goto send_err_free;
...
scif_user_recv(scif_epd_t epd, void *msg, int len, int flags)
{
	struct endpt *ep = (struct endpt *)epd;
	/*
	 * recv_len - Total number of bytes received.
	 * loop_len - Number of bytes to be received in this iteration.
	 * chunk_len - Number of bytes allocated for temporary
	 *		kernel buffer aka "chunk" which will be reused
	 *		for every iteration.
	 */
	int err = 0, recv_len = 0, loop_len, chunk_len;
	char *tmp = msg;
	int maxlocalbuflen = (1 << (MAX_ORDER + PAGE_SHIFT - 1));
	...
	mutex_lock(&ep->sendlock);
	while (recv_len != len) {
		loop_len = min(maxlocalbuflen, len - recv_len);
		if ((err = _scif_recv(epd, msg,
			loop_len, flags)) < 0)
			goto recv_err_free;

		if (copy_to_user(tmp, msg, err)) {
			err = -EFAULT;
			goto recv_err_free;
		}

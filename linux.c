...
extern int mic_crash_dump_enabled;
module_param_named(crash_dump, mic_crash_dump_enabled, int, 0600);
MODULE_PARM_DESC(mic_crash_dump_enabled, "MIC Crash Dump enabled.");

#define XPU_FW_PATH "/lib/firmware/mic/uos.img"
...
set_sysfs_entries(mic_ctx_t *mic_ctx)
{
	...
	mic_ctx->sysfs_info.maint_image =
		kmalloc(strlen(K1OM_XPU_MAINTENANCE_MODE_FW_PATH) + 1, GFP_KERNEL);
	strcpy(mic_ctx->sysfs_info.maint_image, K1OM_XPU_MAINTENANCE_MODE_FW_PATH);
}
...
free_sysfs_entries(mic_ctx_t *mic_ctx)
{
	...
	if (mic_ctx->sysfs_info.maint_image != NULL)
		kfree(mic_ctx->sysfs_info.maint_image);
	if (mic_ctx->initramfs)
...
mic_get_file_size(const char* fn, uint32_t* file_len)
{
	...
	if(fn != NULL)
		...
		filp = filp_open(XPU_FW_PATH, 0, 0);
...
void
create_online_device(int brdnum)
{
	int ret;
	bd_info_t *bd_info;
	struct device *mic_online;

	printk("\ncreating online device for bd[%d]\n", brdnum);
	bd_info = mic_data.dd_bi[brdnum];

	// create
	mic_online = device_create(
			mic_lindata.dd_class, NULL,
			mic_lindata.dd_dev + (mic_data.dd_numdevs + 2) + bd_info->bi_ctx.bi_id,
			NULL, "mic_online%d", bd_info->bi_ctx.bi_id);
	ret = sysfs_create_group(&mic_online->kobj, &bd_attr_group);

	// destroy
	sysfs_remove_group(&mic_online->kobj, &bd_attr_group);
	device_destroy(mic_lindata.dd_class,
		       mic_lindata.dd_dev + (mic_data.dd_numdevs + 2) + bd_info->bi_ctx.bi_id);
